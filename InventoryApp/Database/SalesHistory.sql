...
Select Top 5 C.card_name, US.[Unit Sold]
from (Select card_id, SUM(quantity) as "Unit Sold" from (Select card_id, set_code, rarity, quantity from ReceiptInfo as R1 inner join 
(Select transaction_id, user_id, items, total_price from dbo.Receipt where date <= '2023-08-13' and date >= '2023-08-01' and user_id = '101164323405038069020') as R2 on R1.transaction_id = R2.transaction_id and R1.user_id = R2.user_id) as Q group by card_id) as US
inner join YGOCardsInfo as C on US.card_id = C.card_id order by US.[Unit Sold] desc

exec SalesHistory '101164323405038069020', '2023-08-01', '2023-08-13'

Select card_id, SUM(quantity) as "Unit Sold" from (Select card_id, set_code, rarity, quantity from ReceiptInfo as R1 inner join 
(Select transaction_id, user_id, items, total_price from dbo.Receipt where date <= '2023-08-13' and date >= '2023-08-01' and user_id = '101164323405038069020') as R2 on R1.transaction_id = R2.transaction_id and R1.user_id = R2.user_id) as Q group by card_id

select * from ReceiptInfo where user_id = '101164323405038069020' order by card_id